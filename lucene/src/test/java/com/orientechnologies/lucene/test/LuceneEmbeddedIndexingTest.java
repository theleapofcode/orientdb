...
public class LuceneEmbeddedIndexingTest extends BaseLuceneTest {
  ...
  private void createPersonSchema() {
    ...
    db.command(
        new OCommandSQL("CREATE index person_mobile on person (`mobile.mobileNumber`) FULLTEXT ENGINE LUCENE STRING"))
        .execute();
  ...
  private void createFooSchema() {
    ...
    db.command(new OCommandSQL("CREATE index foo_address on foo (`address.city`) FULLTEXT ENGINE LUCENE STRING"))
        .execute();
    db.command(new OCommandSQL("CREATE index foo_xyz on foo (`xyz.asd.zxc`) FULLTEXT ENGINE LUCENE STRING")).execute();
  }
  ...
  private void createDemoSchema() {
    ...
    db.command(new OCommandSQL("CREATE index demo_elist on demo (`elist.city`) FULLTEXT ENGINE LUCENE STRING"))
        .execute();
    db.command(new OCommandSQL("CREATE index demo_eset on demo (`eset.city`) FULLTEXT ENGINE LUCENE STRING")).execute();
    db.command(
        new OCommandSQL("CREATE index demo_emap on demo (`emap[homeAddress].city`) FULLTEXT ENGINE LUCENE STRING"))
        .execute();
  }
  ...
  public void testLink() throws Exception {
    List<ODocument> qResult = db.command(new OCommandSQL("select * from person where mobile.mobileNumber lucene '123'"))
        .execute();
    Assert.assertEquals(1, qResult.size());
  }
  ...
  public void testEmbeddedWithLinkedClass() throws Exception {
    List<ODocument> qResult = db.command(new OCommandSQL("select * from foo where address.city lucene 'NY'")).execute();
    Assert.assertEquals(1, qResult.size());
  }
  ...
  public void testEmbeddedListOfLinkedClass() throws Exception {
    List<ODocument> qResult = db.command(new OCommandSQL("select * from demo where elist.city lucene 'BLR'")).execute();
    Assert.assertEquals(1, qResult.size());
  }

  @Test
  public void testEmbeddedSetOfLinkedClass() throws Exception {
    List<ODocument> qResult = db.command(new OCommandSQL("select * from demo where eset.city lucene 'SJ'")).execute();
    Assert.assertEquals(1, qResult.size());
  }

  @Test
  public void testEmbeddedMapOfLinkedClass() throws Exception {
    List<ODocument> qResult = db.command(new OCommandSQL("select * from demo where emap[homeAddress].city lucene 'N*'"))
        .execute();
    Assert.assertEquals(1, qResult.size());
  }
